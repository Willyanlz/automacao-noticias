...
Se o campo `numeroErros` estiver preenchido no node `Configurar Cliente` do workflow que falhou, o workflow global le essa configuracao do proprio workflow original e tenta enviar um alerta pelo WhatsApp via Evolution API. Se `numeroErros` estiver vazio, nada e enviado no WhatsApp e o diagnostico fica apenas nos logs/output do n8n.
